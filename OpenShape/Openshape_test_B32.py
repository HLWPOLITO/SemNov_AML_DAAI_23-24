...
#Copied from https://huggingface.co/OpenShape/openshape-demo-support/tree/main/openshape
import torch
...
sys.path.insert(0, '/content/drive/MyDrive/Openshape/SemNov_AML_DAAI_23-24/OpenShape')
...
class PointPatchTransformer(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, return_penultimate=False):
        ...
        #defining the features inside the ppt again in the exact same way that defined in the projected
...
def G14(s):
    ...
    model_path = os.path.join('/content/drive/My Drive/Openshape/SemNov_AML_DAAI_23-24/pointbert-vitg14-rgb','model.pt')
...
def L14(s):
    ...
    model_path = os.path.join('/content/drive/My Drive/Openshape/SemNov_AML_DAAI_23-24/pointbert-vitl14-rgb','model.pt')
    s = torch.load(model_path)
...
def B32(s):
    ...
    model_path = os.path.join('/content/drive/My Drive/Openshape/SemNov_AML_DAAI_23-24/pointbert-vitb32-rgb','model.pt')
    s = torch.load(model_path)
    dic = model.load_state_dict(module(s, 'pc_encoder'))
    print(dic)
    return model
...
def eval_ood_md2sonn(opt, config, s):
    ...
    model_path = os.path.join('/content/drive/My Drive/Openshape/SemNov_AML_DAAI_23-24/pointbert-vitb32-rgb','model.pt')
    s = torch.load(model_path)
    model = B32(s)#Select an instance of model
...
def eval_OOD_with_feats(model, train_loader, src_loader, tar1_loader, tar2_loader, save_feats=None):
    ...
    ################################################
    #We only evaluated the models with Euclidean distance method, so we've delated other methods
...
def main():
    ...
    if args.script_mode.startswith('train'):
        ...
        eval_ood_md2sonn(args, config, "some string")#Create a position for the parameter s in B32(s)
